...
const Portrait = () => {
  ...
  useEffect(() => {
    ...
    // Dynamically import all images from the portrait folder
    const loadImages = async () => {
      try {
        // For Vite - use import.meta.glob with eager loading
        const imageModules = import.meta.glob(
          "../../assets/images/portrait/*.{jpg,jpeg,png,webp,gif,JPG,JPEG,PNG,WEBP,GIF}",
          { eager: true }
        );
        ...
        console.log("Loaded images:", loadedImages); // Debug log
        setImages(loadedImages);

        // For Webpack/CRA - uncomment this and comment out the above
        /*
        const req = require.context('../../assets/images/portrait', false, /\.(png|jpe?g|webp|gif)$/i);
        ...
        {images && images.length > 0 ? (
          images.map((image, index) => (
            ...
            >
              <div className="w-full p-2">
                <img
                  src={image.src}
                  alt={image.alt || `Portrait ${index + 1}`}
          ...
          <div className="col-span-full text-center py-20">
            <p className="text-gray-400 text-xl">
              {images === null
                ? "Loading images..."
                : "No images found in portrait folder"}
            </p>
            <p className="text-gray-500 text-sm mt-2">
              Add images to: src/assets/images/portrait/
            </p>
          </div>
        )}
      </div>
      ...
      {images.length > 0 && (
        <div className="text-center mt-8">
          <p className="text-gray-300">
            Displaying {images.length} portrait{images.length !== 1 ? "s" : ""}
          </p>
        </div>
      )}
    </>
