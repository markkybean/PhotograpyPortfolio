...
const Pets = () => {
  ...
  useEffect(() => {
    ...
    // Dynamically import all images from the pets folder
    const loadImages = async () => {
      try {
        // For Vite - use import.meta.glob with eager loading
        const imageModules = import.meta.glob(
          "../../assets/images/pets/*.{jpg,jpeg,png,webp,gif,JPG,JPEG,PNG,WEBP,GIF}",
          { eager: true }
        );
        ...
        console.log("Loaded pets images:", loadedImages); // Debug log
        setImages(loadedImages);

        // For Webpack/CRA - uncomment this and comment out the above
        /*
        const req = require.context('../../assets/images/pets', false, /\.(png|jpe?g|webp|gif)$/i);
    ...
    <>
      ...
      <div className="mb-12 text-center" data-aos="fade-down">
        <div className="flex justify-center mb-4">
          <PetsIcon className="text-white text-6xl" />
        ...
        <p className="text-xl text-gray-200 mb-2">
          Capturing your furry friends' best moments
        </p>
        <p className="text-lg text-gray-200">
          Memories with your beloved companions
        </p>
      </div>
        ...
        {images && images.length > 0 ? (
          ...
          <div className="col-span-full text-center py-20">
            <p className="text-gray-400 text-xl">
              {images === null
                ? "Loading images..."
                : "No images found in pets folder"}
            </p>
            <p className="text-gray-500 text-sm mt-2">
              Add images to: src/assets/images/pets/
            </p>
          </div>
        )}
      </div>
      ...
      {images.length > 0 && (
        <div className="text-center mt-8">
          <p className="text-gray-300">
            Displaying {images.length} pet photo
            {images.length !== 1 ? "s" : ""}
          </p>
        </div>
      )}
