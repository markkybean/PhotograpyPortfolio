...
const Astro = () => {
  ...
  useEffect(() => {
    ...
    // Dynamically import all images from the astro folder
    const loadImages = async () => {
      try {
        // For Vite - use import.meta.glob with eager loading
        const imageModules = import.meta.glob(
          "../../assets/images/astro/*.{jpg,jpeg,png,webp,gif,JPG,JPEG,PNG,WEBP,GIF}",
          { eager: true }
        );
        ...
        console.log("Loaded astro images:", loadedImages); // Debug log
        setImages(loadedImages);

        // For Webpack/CRA - uncomment this and comment out the above
        /*
        const req = require.context('../../assets/images/astro', false, /\.(png|jpe?g|webp|gif)$/i);
    ...
    <>
      ...
      <div className="mb-12 text-center" data-aos="fade-down">
        <div className="flex justify-center mb-4">
          <NightsStayIcon className="text-white text-6xl" />
          <StarIcon className="text-white text-4xl ml-2" />
        </div>
        <h1 className="md:text-3xl sm:text-2xl font-bold text-white mb-4">
          Astro Photography
        </h1>
        ...
        {images && images.length > 0 ? (
          ...
          <div className="col-span-full text-center py-20">
            <p className="text-gray-400 text-xl">
              {images === null
                ? "Loading images..."
                : "No images found in astro folder"}
            </p>
            <p className="text-gray-500 text-sm mt-2">
              Add images to: src/assets/images/astro/
            </p>
          </div>
        )}
      </div>
      ...
      {images.length > 0 && (
        <div className="text-center mt-8">
          <p className="text-gray-300">
            Displaying {images.length} astro photo
            {images.length !== 1 ? "s" : ""}
          </p>
        </div>
      )}
