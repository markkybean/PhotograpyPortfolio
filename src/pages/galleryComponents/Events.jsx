    ...
    const loadImages = async () => {
      try {
        ...
        const imageModules = import.meta.glob(
          "../../assets/images/events/*.{jpg,jpeg,png,webp,gif,JPG,JPEG,PNG,WEBP,GIF}",
          { eager: true }
        );
        ...
        console.log("Loaded events images:", loadedImages); // Debug log
        setImages(loadedImages);

        // For Webpack/CRA - uncomment this and comment out the above
        /*
        const req = require.context('../../assets/images/events', false, /\.(png|jpe?g|webp|gif)$/i);
    ...
    <>
      ...
      <div className="mb-12 text-center" data-aos="fade-down">
        <div className="flex justify-center mb-4">
          <EventIcon className="text-white text-6xl" />
        ...
        {images && images.length > 0 ? (
          ...
          <div className="col-span-full text-center py-20">
            <p className="text-gray-400 text-xl">
              {images === null
                ? "Loading images..."
                : "No images found in events folder"}
            </p>
            <p className="text-gray-500 text-sm mt-2">
              Add images to: src/assets/images/events/
            </p>
          </div>
        )}
      </div>
      ...
      {images.length > 0 && (
        <div className="text-center mt-8">
          <p className="text-gray-300">
            Displaying {images.length} event photo
            {images.length !== 1 ? "s" : ""}
          </p>
        </div>
      )}
